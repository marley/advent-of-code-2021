...
const predictLastBingo = (data) => {
  let { randomDraws, boards } = data;
  const bingoTracker = {}; // keeps track of how many x's are on each row and column.  If sum = 5, then that's a bingo.
  const boardsMarked = new Set();
  let lastBingo = { markedBoard: [], bingo: "" }; // starts out with all boards and slowly loses them one by one until there is only one left
  for (const randomDraw of randomDraws) {
    for (const boardNumber in boards) {
      let { markedBoard, bingo } = updateBoardAndCheckForBingo(
        randomDraw,
        boards[boardNumber],
        boardNumber,
        bingoTracker
      ...
      if (bingo >= 0) {
        // keep track of last board to have a bingo
        if (!boardsMarked.has(boardNumber)) {
          lastBingo = { markedBoard, bingo };
          boardsMarked.add(boardNumber);
        }
      }
      boards[boardNumber] = markedBoard;
    }
  }
  return calculateScore(lastBingo.markedBoard, lastBingo.bingo);
};
...
// console.log(predictLastBingo(testData));

const data = bingoDataToObject(rawData);
console.log(predictLastBingo(data));
